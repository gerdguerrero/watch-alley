-- Watch Alley: Restore auto-ID generation in admin_upsert_watch (2026-05-25)
--
-- Migration 0018 (applied directly, no repo file) added category + badges to
-- admin_upsert_watch but replaced the entire function body, accidentally
-- dropping the auto-ID generation (twa-NNN) that had been present since
-- migration 0006. The INSERT path was missing `id` entirely, meaning new
-- watches could never be created.
--
-- This migration rewrites admin_upsert_watch with:
--   ✓ Auto-ID generation (restored from 0014)
--   ✓ Category validation
--   ✓ Badges JSONB normalization
--   ✓ Both category and badges persisted on INSERT and UPDATE
...
begin
  ...
  -- Auto-generate twa-NNN ID for new watches
