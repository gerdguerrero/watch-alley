-- Watch Alley: Allow photo-less inventory drafts (2026-06-04)
--
-- The admin workflow can now create the listing details on a laptop first,
-- save the row as a private draft, then add photos from mobile before
-- publishing. Public listings still require at least one photo.
--
-- Draft is represented by the existing listing visibility state:
--   published = false  => Draft
--   published = true   => Live/published

-- The original bootstrap constraint required at least one image for every row.
-- Keep that rule for public listings, but allow empty arrays for drafts.
...
-- Recreate the admin upsert RPC so empty image arrays persist as '{}'::text[]
-- instead of NULL, which would violate the images NOT NULL column.
