-- Watch Alley provenance story (2026-04-28)
--
-- Adds a free-form provenance text column to watch_alley.watches so each
-- listing can carry the editorial story that the heritage-craft brand
-- promises: prior owner, service notes, why this piece was curated,
-- anything that turns a spec sheet into a record.
--
...
comment on column watch_alley.watches.provenance is
  'Optional editorial provenance story: prior owner, history, service notes, curation rationale. Rendered in the product detail modal as a Spectral paragraph between description and price.';

-- ===========================================================================
-- 2. Recreate public.watches view to expose provenance
-- ===========================================================================
-- Postgres does not allow create-or-replace to insert a column in the middle
-- of an existing view's column list. Drop first, then recreate.
...
comment on view public.watches is
  'Read-only view of watch_alley.watches exposed to PostgREST. Writes go through admin RPCs.';
grant select on public.watches to anon, authenticated;

-- ===========================================================================
-- 3. Update admin_upsert_watch to accept and persist provenance
-- ===========================================================================
