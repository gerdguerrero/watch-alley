...
The current Supabase project is the source of truth for both the admin dashboard and public storefront inventory. The admin bridge writes through allowlisted RPCs; the Next.js storefront reads the public `public.watches` view/server-side table live and keeps `next/public/data/watches.json` only as a resilience fallback for legacy surfaces.
...
4. Copy your project's **URL** and **publishable anon key** into `next/.env.local` and into the placeholder constants near the top of `next/public/scripts/admin.js` while the legacy admin bridge exists.
5. Visit `/admin`, sign up, sign in, then add inventory rows.
6. Visit the homepage and confirm the storefront reads from the live `public.watches` view. Refresh `next/public/data/watches.json` only when you want to update the legacy fallback snapshot.
...
Next.js storefront pages + legacy admin bridge
        │
        ▼ fallback only if live read is unavailable
next/public/data/watches.json
```
...
4. Optional: refresh `next/public/data/watches.json` when you deliberately want to update the legacy fallback snapshot.
...
| `slug` | text unique | URL-safe kebab-case. Powers `/watch/<slug>` pages and Copy share link. |
...
Local bridge images live under `next/public/watch-assets/`. New production images should prefer Supabase Storage URLs so `next/image` can optimize them without committing binary assets.
...
When adding a new listing during the bridge period, either upload through the admin workflow or add the source image under `next/public/watch-assets/` and reference `/watch-assets/name.png` in `primary_image`/`images[]`. The Next storefront renders through `next/image`.

## Admin update workflow

Admin operations happen at `/admin` through a static bridge in `next/public/admin/index.html`. The page is private:
...
- [ ] `primary_image` is a valid Supabase Storage URL or exists in `next/public/watch-assets/`; image looks right at card and detail-page sizes.
...
4. `next/public/data/watches.json` remains a safe fallback for legacy/static bridge behavior if a live read is temporarily unavailable.
