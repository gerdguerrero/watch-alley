...
**Supabase project `trqgzrylysybgipcejwe`, schema `watch_alley`, table `watches`.**

The homepage and trust pages still read inventory from `public/data/watches.json` at runtime. That file is no longer hand-edited — it is generated from Supabase by `pnpm sync:watches`. This keeps the homepage fast and offline-friendly while making real CRUD possible.
