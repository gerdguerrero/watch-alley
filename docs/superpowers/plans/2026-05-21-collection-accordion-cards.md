# Collection Accordion Cards Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the 3-column grid in `CollectionSection` with a horizontal accordion card layout — collapsed strips expand on hover (desktop) or tap (mobile), showing watch image background + name + category.

**Architecture:** Single file rewrite of `components/collection-section.tsx`. Replace `CollectionCard` with `AccordionCard`. Animate `flexGrow` via Framer Motion on desktop; animate `height` on mobile. All other section elements (title, filter pills, ambient glow) are unchanged.

**Tech Stack:** Next.js 16, React 19, Framer Motion 12, Tailwind CSS 4, Lucide React, existing `useIsMobile` hook

---

### Task 1: Rewrite `CollectionCard` as `AccordionCard`

**Files:**
- Modify: `components/collection-section.tsx`

- [ ] **Step 1: Add icon field to `collections` array**

Replace the existing `collections` constant (lines 11–33) with:
...
- [ ] **Step 2: Replace `CollectionCard` component with `AccordionCard`**

Delete the entire `CollectionCard` function (lines 37–167) and replace with:
...
At the top of `components/collection-section.tsx`, update the import line:
...
Note: remove the `useMotionValue, useSpring, Image` imports that are no longer used.

- [ ] **Step 4: Update `CollectionSection` — add state + wire `AccordionCard`**

In `CollectionSection`, add `activeId` state and `isMobile`, then replace the grid with the accordion container:
...
Run:
```bash
cd C:\GitHub\watch-alley-v2 && npx tsc --noEmit
```

Expected: no errors. If `LucideIcon` type import causes issues, replace with `React.ComponentType<{ className?: string }>`.
...
Open `http://localhost:3000` and scroll to the collection section. Verify:
- Desktop: 3 cards in a horizontal row, first card wide, others narrow
- Hover each card — it expands smoothly, others collapse
- Mobile (resize < 768px): cards stack vertically, tap to expand

- [ ] **Step 7: Commit**

```bash
git add components/collection-section.tsx
git commit -m "feat: replace collection grid with horizontal accordion cards"
```
