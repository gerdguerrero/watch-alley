-- Admin allowlist management RPCs.
-- list / remove are pure DB operations; invite goes through the
-- invite-admin Edge Function so it can call auth.admin.inviteUserByEmail
-- with the service-role key (which must never live in the browser).
...
-- Remove an admin. Refuses to remove the calling admin (no self-deletion).
-- Refuses to remove the last admin (no bricking the system).
...
begin
  ...
  -- Refuse if removing this row would leave 0 admins.
...
-- entry atomically with the auth.admin.inviteUserByEmail() call. Unlike the
-- other admin RPCs this one is callable only by service_role (the Edge
-- Function), since the caller is doing the auth check itself before calling
-- the privileged auth.admin API.
...
declare
  normalized text;
begin
  -- service_role bypass-rls calls land here with empty auth.email().
  -- We accept those (the Edge Function has already verified caller identity).
  -- Anonymous calls and plain-authenticated calls are rejected.
