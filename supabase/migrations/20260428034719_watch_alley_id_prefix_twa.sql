-- Rename watch IDs from wa-* to twa-* (project name is "the-watch-alley").
-- inquiries.watch_id has ON DELETE SET NULL, no other tables reference it,
-- so a simple in-place update works. We do it in one transaction.
--
-- Also update the admin_upsert_watch auto-id generator to use twa- prefix.

update watch_alley.watches
set id = 'twa-' || substr(id, 4)
where id ~ '^wa-[0-9]+$';

-- The inquiries.watch_id FK is ON DELETE SET NULL but here we update parent IDs.
-- Postgres needs to cascade those updates manually since we didn't declare
-- ON UPDATE CASCADE. Update them explicitly:
update watch_alley.inquiries
set watch_id = 'twa-' || substr(watch_id, 4)
where watch_id ~ '^wa-[0-9]+$';

-- Replace admin_upsert_watch with twa-prefixed auto-id.
...
-- Update column comment to reflect new prefix.
