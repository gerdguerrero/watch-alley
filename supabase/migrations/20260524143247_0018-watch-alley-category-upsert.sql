-- Update admin_upsert_watch to accept and persist category
...
BEGIN
  ...
  IF cat IS NOT NULL AND cat NOT IN ('brand-new', 'pre-owned', 'limited-edition') THEN
    RAISE EXCEPTION 'Invalid category: %', cat USING ERRCODE = '22023';
  END IF;
  ...
  ELSE
    ...
    INSERT INTO watch_alley.watches (
      ...
      condition_label, badge, movement, case_size, inclusion_set, material,
      ...
      payload->>'conditionLabel', payload->>'badge', payload->>'movement',
      payload->>'caseSize', payload->>'set', payload->>'material',
      payload->>'edition', payload->>'description', payload->>'disclosure',
      nullif(payload->>'provenance', ''), payload->>'primaryImage',
