-- Watch Alley structured inquiries (replaces mailto void).
-- Public submits via submit_inquiry() RPC; admin reads via admin_list_inquiries()
-- and updates status via admin_update_inquiry_status().
...
comment on table watch_alley.inquiries is 'Buyer inquiries. Public can insert via submit_inquiry RPC. Admin reads/updates via admin_* RPCs. Status workflow: new -> contacted -> viewing -> reserved -> sold/lost.';
...
-- and shape constraints baked into the table CHECKs above.
...
begin
  ...
  -- Return only safe public fields (no PII echo).
...
-- Admin list (paginated, status-filtered).
...
-- Admin metrics dashboard (the "one metric" — inquiry-to-sale conversion).
