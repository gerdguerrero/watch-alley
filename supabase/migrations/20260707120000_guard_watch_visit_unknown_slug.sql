-- Watch Alley: reject view-tracking for slugs that are not real watches.
--
-- The public /api/track-watch-view/[slug] route forwards whatever slug it is
-- given to this RPC, which previously upserted a counter row for ANY non-empty
-- string. That let placeholder/bot slugs (e.g. a QA "test-slug" smoke test)
-- create phantom rows in watch_page_views and top the admin "Most Viewed" list
-- with entries that have no matching inventory. We now short-circuit unless the
-- slug exists in public.watches, so a fake slug records nothing at all (no view,
-- no country, no visitor id).
...
begin
  ...
  -- Only real inventory can be "viewed". Unknown slugs (test harnesses, bots,
  -- stale links to deleted watches) are ignored entirely so analytics stay clean.
  if not exists (select 1 from public.watches w where w.slug = resolved_slug) then
...
comment on function public.service_record_watch_visit(jsonb) is
  'Service-role-only atomic logger for public watch page views, visitor countries, and anonymous visitor ids. Ignores slugs that do not exist in public.watches.';
