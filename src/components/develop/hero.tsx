  ...
  return (
    <section ref={sectionRef} className="relative h-screen bg-[#0a0a0a] text-zinc-100 overflow-hidden">
      ...
      <div ref={bottomLeftRef} className="absolute bottom-12 md:bottom-16 left-6 md:left-12 lg:left-20 z-20 max-w-lg opacity-0">
        ...
        <MotionLink
          href="/available"
          className="mt-6 px-6 py-3 bg-zinc-100 text-zinc-900 text-xs tracking-[0.15em] uppercase inline-flex items-center gap-3 group overflow-hidden relative"
      ...
      <div ref={bottomRightRef} className="absolute bottom-12 md:bottom-16 right-6 md:right-12 lg:right-20 z-20 max-w-sm text-right hidden md:block opacity-0">
        {featured ? (
          <Link href={`/watch/${featured.slug}`} className="block group">
            <p className="text-[10px] tracking-[0.3em] text-amber-500/80 uppercase mb-3 font-mono">
              ◆ Featured · Available now
            </p>
            <p className="font-serif text-xl text-zinc-100 leading-tight mb-1">
              {featured.brand}
              <br />
              <span className="italic text-zinc-300">{featured.name}</span>
            </p>
            <p className="font-serif text-lg text-amber-500 mt-3">
              {formatPhp(featured.price)}
            </p>
            <span className="mt-4 inline-flex items-center gap-2 text-[11px] tracking-[0.2em] uppercase text-zinc-400 group-hover:text-amber-400 transition-colors">
              View this piece
