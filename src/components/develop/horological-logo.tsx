...
interface HorologicalLogoProps {
  /** Mode of animation:
   *  - 'mount-sweep': Runs a Clockwork (1-Hour) sweep once on component mount.
   *  - 'hover-sweep': Runs a fast Clockwork (1-Hour) sweep when hovered.
   *  - 'static': Renders the static lockup.
   */
  mode?: "mount-sweep" | "hover-sweep" | "static";
  /** Optional class name for the outer container. */
  className?: string;
  /** Width override (e.g. 56 or "100%"). */
  width?: number | string;
...
export function HorologicalLogo({
  ...
  useGSAP(
    () => {
      if (mode !== "mount-sweep") return;

      const hingeGlow = hingeGlowRef.current;

      // Hide the wordmarks initially using a horizontal wipe inset
      gsap.set(
        [
          ".wa-wordmark-the",
          ".wa-wordmark-watch-w",
      ...
      gsap.set([".wa-left-leg", ".wa-right-leg"], {
        opacity: 1,
        scale: 1,
      });
      ...
      // Minute Hand (Left Leg) rotates exactly once (360 degrees) around fixed center (184.3, 35.4)
      tl.fromTo(
        ".wa-left-leg",
        { rotate: -360, svgOrigin: "184.3 35.4" },
        { rotate: 0, svgOrigin: "184.3 35.4", duration: 1.5, ease: "power3.inOut" },
        0.15
      );

      // Hour Hand (Right Leg) rotates exactly 1 hour (30 degrees) around fixed center (184.3, 35.4)
      tl.fromTo(
        ".wa-right-leg",
        { rotate: -30, svgOrigin: "184.3 35.4" },
        { rotate: 0, svgOrigin: "184.3 35.4", duration: 1.5, ease: "power3.inOut" },
        0.15
      );

      // Trigger hinge connect snap glow
      tl.add(() => {
        if (hingeGlow) {
          gsap.fromTo(
            hingeGlow,
      ...
      // Sequential windshield-wipe text reveals following the hand movement
      tl.fromTo(
        ".wa-wordmark-the",
        { clipPath: "inset(0 100% 0 0)" },
  ...
  const handleMouseEnter = () => {
    ...
    // Snappy Clockwork (1-Hour) sweep with premium elastic overshoot
    const tl = gsap.timeline();
    activeTimeline.current = tl;
    ...
    tl.fromTo(
      ".wa-left-leg",
      { rotate: -360, svgOrigin: "184.3 35.4" },
      { rotate: 0, svgOrigin: "184.3 35.4", duration: 0.85, ease: "back.out(1.4)" },
      0
    );
      ...
      if (hingeGlow) {
        gsap.fromTo(
          ...
          { scale: 0.1, opacity: 0 },
          {
            ...
            ease: "power2.out",
            onComplete: () => {
              gsap.to(hingeGlow, { opacity: 0, duration: 0.3, scale: 2.0, ease: "power2.in" });
            },
          }
        );
      }
  ...
  return (
    ...
    >
      {/* Subtle Hinge Glow Center Ring (concentric with joint at 184.3, 35.4)
          viewBox size is 490x365, so pivot sits at x=37.6% (184.3/490), y=9.7% (35.4/365). */}
      <div
        ref={hingeGlowRef}
        className="absolute rounded-full bg-[#BD9A32] blur-md pointer-events-none opacity-0 z-10 w-[7%] h-[9.3%]"
        style={{
          left: "34.1%", // slightly offset for optical center alignment
          top: "5%",
      ...
      >
        <g fill="none">
          {/* THE wordmark */}
          <g className="wa-wordmark-the" fill="#BD9A32" style={{ willChange: "clip-path" }}>
          ...
          <g className="wa-wordmark-watch-w" fill="#FFFFFF" style={{ willChange: "clip-path" }}>
          ...
          <g className="wa-wordmark-watch-tch" fill="#FFFFFF" style={{ willChange: "clip-path" }}>
          ...
          <g className="wa-compass" fill="#BD9A32">
            <path
              className="wa-left-leg"
          ...
          {/* LLEY wordmark */}
          <g className="wa-wordmark-lley" fill="#BD9A32" style={{ willChange: "clip-path" }}>
