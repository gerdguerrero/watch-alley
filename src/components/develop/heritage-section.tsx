...
const FALLBACK_HERO =
  'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Elegant%20luxury%20watch%20boutique%20interior.png-sLmmCilQ5pezpoIZITwGSXhTLEueCp.jpeg'
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
            ...
            >
              ...
              >
                <Image
                  src={latestPost?.heroImage || FALLBACK_HERO}
                  alt={latestPost?.title || 'Watch Alley boutique interior'}
                  fill
                ...
                >
                  <span className="text-[10px] tracking-[0.3em] text-amber-500/80 uppercase block mb-1">
                    {latestPost ? 'Latest from the Journal' : 'Established'}
                  </span>
                  <span className="text-xl md:text-2xl font-serif text-white block max-w-[260px] leading-tight">
                    {latestPost?.title ?? '2026'}
                  </span>
                </motion.div>
              </Link>
            ...
            >
              ...
              >
                ...
                {/* Stats row — pulled from the real inventory + sold archive when available. */}
                <div className="flex gap-12 pt-8 border-t border-zinc-800/50">
                  {[
                    {
                      value:
                        typeof inventorySize === 'number'
                          ? `${inventorySize}`
                          : '50+',
                      label: 'Available Now',
                    },
                    {
                      value:
                        typeof soldSize === 'number' ? `${soldSize}+` : '200+',
                      label: 'Pieces Placed',
                    },
                  ].map((stat, i) => (
                    <motion.div
                ...
                {/* CTA — opens the Journal index. */}
                <MotionLink
                  href="/journal"
                  className="mt-12 group inline-flex items-center gap-4"
                  whileHover={{ x: 8 }}
